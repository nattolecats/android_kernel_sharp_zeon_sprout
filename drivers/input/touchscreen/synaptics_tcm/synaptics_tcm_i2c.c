		...
		unsigned short addr, unsigned char *data, unsigned int length)
{
	...
	for (attempt = 0; attempt < XFER_ATTEMPTS; attempt++) {
		if (i2c_transfer(i2c->adapter, msg, 2) == 2) {
			...
			i2c_communication_log(&i2c->dev, I2C_IO_WRITE, msg[0].buf, msg[0].len);
			i2c_communication_log(&i2c->dev, I2C_IO_READ, msg[1].buf, msg[1].len);
		...
		unsigned short addr, unsigned char *data, unsigned int length)
{
	...
	for (attempt = 0; attempt < XFER_ATTEMPTS; attempt++) {
		if (i2c_transfer(i2c->adapter, &msg, 1) == 1) {
			...
			i2c_communication_log(&i2c->dev, I2C_IO_WRITE, msg.buf, msg.len);
		...
		unsigned int length)
{
	...
	for (attempt = 0; attempt < XFER_ATTEMPTS; attempt++) {
		if (i2c_transfer(i2c->adapter, &msg, 1) == 1) {
			...
			i2c_communication_log(&i2c->dev, I2C_IO_READ, msg.buf, msg.len);
