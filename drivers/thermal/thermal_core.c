...
#define XO_THERM_TYPE          "xo_therm"
#define PA1_THERM_NAME         "pa_therm1"
...
#define USB_THERM_NAME         "usb_therm"
#define PMIC_THERM_NAME        "die_temp"
	...
	int passive_delay, int polling_delay)
{
	...
	if (!strcmp(tz->type, MSM_THERM_NAME) || !strcmp(tz->type, LCD_THERM_NAME) ||
		!strcmp(tz->type, PA1_THERM_NAME) || !strcmp(tz->type, CAM_THERM_NAME) ||
	...
	else if (!strcmp(tz->type, XO_THERM_NAME)) {
		if (sysfs_create_link(&(tz->device.class)->p->subsys.kobj, &tz->device.kobj, XO_THERM_TYPE)) {
			pr_debug("fail to create link sys file:%s\n", XO_THERM_TYPE);
		}
	}
#endif /* CONFIG_BATTERY_SHARP */
