...
static int mmc_select_hs400(struct mmc_card *card)
{
	...
	err = __mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
			   ...
			   card->ext_csd.generic_cmd6_time,
			   true, false, true);
#if defined(CONFIG_SHARP_MMC_EMMC) && defined(CONFIG_ANDROID_ENGINEERING)
	pr_info( "%s: %s : DS : 0x%x\n",
		mmc_hostname(card->host),
		__func__,
		card->drive_strength);
#endif /* CONFIG_SHARP_MMC_EMMC && CONFIG_ANDROID_ENGINEERING */
	if (err) {
...
#ifndef CONFIG_SHARP_MMC_EMMC_DRIVESTRENGTH
	} else if (card->mmc_avail_type & EXT_CSD_CARD_TYPE_HS200) {
		err = mmc_select_hs200(card);
#endif /* CONFIG_SHARP_MMC_EMMC_DRIVESTRENGTH */
...
reinit:
	 ...
	 */
#ifdef CONFIG_SHARP_MMC_EMMC_DRIVESTRENGTH
	card->drive_strength = 0x0;
#endif /* CONFIG_SHARP_MMC_EMMC_DRIVESTRENGTH */
...
#ifdef CONFIG_SHARP_MMC_EMMC_CACHE_DISABLE
	card->quirks |= MMC_QUIRK_CACHE_DISABLE;
#endif /* CONFIG_SHARP_MMC_EMMC_CACHE_DISABLE */
