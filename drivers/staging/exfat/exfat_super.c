 ...
 *         :  bits :     value
 * datetime:  0 -  4: sec	(0 -  29) 2sec counts
 * datetime:  5 - 10: min	(0 -  59)
 * datetime: 11 - 15: hour	(0 -  23)
 * datetime: 16 - 20: day	(1 -  31)
 * datetime: 21 - 24: month	(1 -  12)
 * datetime: 25 - 31: year	(0 - 127) from 1980
 */
#define SECS_PER_MIN    60
#define SECS_PER_HOUR   (60 * 60)
#define SECS_PER_DAY    (SECS_PER_HOUR * 24)
/* days between 1970.1.1 and 1980.1.1 (2 leap days) */
...
/* UTC offset */
#define UTC_OFFSET_VALID_MASK       0x80
#define UTC_OFFSET_SIGN_FLAG_MASK   0x40
#define UTC_OFFSET_VALUE_MASK       0x3F
#define UTC_OFFSET_MAX_VALUE        0x40
#define IS_UTC_VALID(offs)          (offs & UTC_OFFSET_VALID_MASK)
#define IS_UTC_OFFSET_MINUS(offs)   (offs & UTC_OFFSET_SIGN_FLAG_MASK)

/* Convert a FAT time/date pair to a UNIX date (seconds since 1 1 70). */
static void exfat_time_fat2unix(struct timespec *ts, struct date_time_t *tp, /*u8 time_cs,*/ u8 utc_offset)
{
    time64_t utc_offs;

	ts->tv_sec = mktime(tp->Year + 1980, tp->Month, tp->Day,
			      tp->Hour, tp->Minute, tp->Second);

    /* Second set to UTC */
    if (IS_UTC_VALID(utc_offset)) {
        utc_offs = utc_offset & UTC_OFFSET_VALUE_MASK;
        if (IS_UTC_OFFSET_MINUS(utc_offset)) {
            utc_offs = (UTC_OFFSET_MAX_VALUE - utc_offs) * -1;
        }
        utc_offs *= 15;
        ts->tv_sec -= utc_offs * SECS_PER_MIN;
    } else {
...
static void exfat_time_unix2fat(struct timespec *ts, struct date_time_t *tp, /*u8 *time_cs,*/ u8 *utc_offset)
{
	time64_t second = ts->tv_sec;
	struct tm tm;

	time_to_tm(second, -sys_tz.tz_minuteswest * 60, &tm);
	...
	exfat_get_utc_offset(utc_offset);
}

void exfat_get_utc_offset(u8 *utc_offset)
{
	if (utc_offset) {
		int offs = (sys_tz.tz_minuteswest / 15) * -1;
		if (offs >= 0) {
			*utc_offset = UTC_OFFSET_VALID_MASK
				| (offs & UTC_OFFSET_VALUE_MASK);
		} else {
			offs *= -1;
			*utc_offset = UTC_OFFSET_VALID_MASK
				| UTC_OFFSET_SIGN_FLAG_MASK
				| ((UTC_OFFSET_MAX_VALUE - offs) & UTC_OFFSET_VALUE_MASK);
		}
	}
}
...
static int ffsReadStat(struct inode *inode, struct dir_entry_t *info)
{
    ...
    info->create_utc_offset = p_fs->fs_func->get_entry_create_utc_offset(ep);
    info->modify_utc_offset = p_fs->fs_func->get_entry_modify_utc_offset(ep);
    info->access_utc_offset = p_fs->fs_func->get_entry_access_utc_offset(ep);
...
static int ffsWriteStat(struct inode *inode, struct dir_entry_t *info)
{
	...
	p_fs->fs_func->set_entry_create_utc_offset(ep, info->create_utc_offset);
	p_fs->fs_func->set_entry_modify_utc_offset(ep, info->modify_utc_offset);
	p_fs->fs_func->set_entry_access_utc_offset(ep, info->access_utc_offset);
...
{
	return hash_64(i_pos, EXFAT_HASH_BITS);
}
...
static int exfat_fill_inode(struct inode *inode, struct file_id_t *fid)
{
	...
	exfat_time_fat2unix(&inode->i_ctime, &info.CreateTimestamp, info.create_utc_offset);
	exfat_time_fat2unix(&inode->i_mtime, &info.ModifyTimestamp, info.modify_utc_offset);
	exfat_time_fat2unix(&inode->i_atime, &info.AccessTimestamp, info.access_utc_offset);
...
static int exfat_write_inode(struct inode *inode, struct writeback_control *wbc)
{
	...
	exfat_time_unix2fat(&inode->i_ctime, &info.CreateTimestamp, &info.create_utc_offset);
	exfat_time_unix2fat(&inode->i_mtime, &info.ModifyTimestamp, &info.modify_utc_offset);
	exfat_time_unix2fat(&inode->i_atime, &info.AccessTimestamp, &info.access_utc_offset);
