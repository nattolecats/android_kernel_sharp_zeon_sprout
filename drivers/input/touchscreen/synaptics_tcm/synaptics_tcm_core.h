...
/* [ Debug ]                                                                  */
/*============================================================================*/
//#define SYNAPTICS_TCM_DEVELOP_MODE_ENABLE

/*============================================================================*/
/* [ Config ]                                                                 */
/*============================================================================*/
#define SYNAPTICS_TCM_SEND_SHTERM_EVENT_ENABLE

//#define SYNAPTICS_TCM_COVER_ENABLE
#if defined(SYNAPTICS_TCM_COVER_ENABLE)
	#include <misc/shflip.h>
#endif /* SYNAPTICS_TCM_COVER_ENABLE */

#define SYNAPTICS_TCM_LPMODE_ENABLE


/*============================================================================*/
/* [ Parameters ]                                                             */
/*============================================================================*/
#define TOUCH_DEFAULT_MAX_X					1079
#define TOUCH_DEFAULT_MAX_Y					2159
#define TOUCH_DEFAULT_MAX_OBJEXTS			5
...
#define LOGE(dev, log, ...) LOGy(dev_err, dev, log, ##__VA_ARGS__)
	...
	int syna_tcm_get_log_enable(void);

	#define LOGD(dev, log, ...) if(syna_tcm_get_log_enable() != 0) LOGx(dev_dbg, dev, log, ##__VA_ARGS__)
	#define LOGI(dev, log, ...) if(syna_tcm_get_log_enable() != 0) LOGx(dev_info, dev, log, ##__VA_ARGS__)
	#define LOGN(dev, log, ...) LOGx(dev_notice, dev, log, ##__VA_ARGS__)
	#define LOGW(dev, log, ...) LOGy(dev_warn, dev, log, ##__VA_ARGS__)

	#define LOG_EVENT(dev, log, ...) \
		dev_notice(dev, log, ##__VA_ARGS__)
#else
	#define LOGD(dev, log, ...)
	#define LOGI(dev, log, ...)
	#define LOGN(dev, log, ...)
...
struct syna_tcm_hcd {
	...
	wait_queue_head_t wait_in_suspend;
	int irq;
	bool init_okay;
	bool do_polling;
	bool in_suspend;
	...
	struct pinctrl_state *int_state_standby;
...
#endif
#if defined(SYNAPTICS_TCM_COVER_ENABLE)
	unsigned int cover_state;
	struct notifier_block cover_notifier;
#endif /* SYNAPTICS_TCM_COVER_ENABLE */
#if defined(SYNAPTICS_TCM_LPMODE_ENABLE)
	unsigned int lpmode_req_state;
	unsigned int lpmode_continuous_req_state;
