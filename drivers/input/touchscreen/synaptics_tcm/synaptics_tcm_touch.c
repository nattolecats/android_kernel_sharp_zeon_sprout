...
static BLOCKING_NOTIFIER_HEAD(touchevent_notifier_list);
...
static void touch_report(void)
{
	...
	if(prev_touch_count > 0 && touch_count == 0){
		blocking_notifier_call_chain(&touchevent_notifier_list, 0, NULL);
	}
	else if(prev_touch_count == 0 && touch_count > 0){
		blocking_notifier_call_chain(&touchevent_notifier_list, 1, NULL);
	}
...
{
	return blocking_notifier_chain_register(&touchevent_notifier_list, nb);
}
EXPORT_SYMBOL(touchevent_register_client);

int touchevent_unregister_client(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&touchevent_notifier_list, nb);
}
