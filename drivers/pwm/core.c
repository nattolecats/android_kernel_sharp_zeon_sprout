...
int pwm_apply_state_u64(struct pwm_device *pwm, struct pwm_state *state)
{
	...
	if (pwm->chip->ops->apply) {
		...
		if (state->period_u64 != pwm->state.period_u64 ||
		    state->duty_cycle_u64 != pwm->state.duty_cycle_u64) {
			err = pwm->chip->ops->config_u64(pwm->chip, pwm,
						     state->duty_cycle_u64,
						     state->period_u64);
			if (err)
				return err;
...
EXPORT_SYMBOL_GPL(pwm_apply_state_u64);
