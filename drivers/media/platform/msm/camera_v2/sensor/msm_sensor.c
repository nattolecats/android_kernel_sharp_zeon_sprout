...
extern int32_t sh_ois_exit(struct msm_sensor_ctrl_t *s_ctrl);
#endif /* CONFIG_SH_OIS */
...
#ifdef SHLOCAL_CAMERA_GLOBAL_FREE
...
/* SHLOCAL_CAMERA_DRIVERS<- */
	...
	case CFG_GET_SENSOR_INIT_PARAMS:
		...
		CDBG("%s:%d init params mode %d pos %d mount %d\n", __func__,
			...
			cdata->cfg.sensor_init_params.sensor_mount_angle);
		break;
/* SHLOCAL_CAMERA_IMAGE_QUALITY-> */ /* Built-in Hybrid AF */
#ifdef CONFIG_SH_OIS
	case SHCFG_WRITE_I2C_ARRAY_WAIT_OIS:
    sh_ois_fw_wait_complete();
#endif /* CONFIG_SH_OIS */
/* SHLOCAL_CAMERA_IMAGE_QUALITY<- */ /* Built-in Hybrid AF */
...
/* SHLOCAL_CAMERA_DRIVERS-> */
	{
		...
		if(conf_array.data_type == MSM_CAMERA_I2C_BYTE_DATA){
			...
			while((i < conf_array.size) && (pos_reg_setting->delay != 0)){
				pos_reg_sec_setting->reg_data_size = cur_size;
				conf_sec_array.delay = pos_reg_setting->delay;
				conf_sec_array.size = array_size;
				
				CDBG("%s reg_addr:0x%0x reg_data:0x%0x delay=%d\n", __func__, pos_reg_setting->reg_addr, pos_reg_setting->reg_data, pos_reg_setting->delay);
				CDBG("%s array_size = %d\n", __func__, array_size);
				...
				i++;
				CDBG("%s i=%d conf_array.size=%d\n", __func__, i, conf_array.size);
			}
			
			pos_reg_setting++;
			
			for (; i < conf_array.size; i++) {
				...
				while((i < conf_array.size) && (pos_reg_setting->delay != 0)){
					...
					pr_err("%s reg_addr:0x%0x reg_data:0x%0x delay=%d\n", __func__, pos_reg_setting->reg_addr, pos_reg_setting->reg_data, pos_reg_setting->delay);
					CDBG("%s array_size = %d\n", __func__, array_size);
			...
			if(reg_setting[conf_array.size - 1].delay != 0){
				pr_err("%s i=%d conf_array.size=%d\n", __func__, i, conf_array.size);
				pr_err("%s reg_addr:0x%0x reg_data:0x%0x delay = %d\n", __func__, reg_setting[conf_array.size - 1].reg_addr, reg_setting[conf_array.size - 1].reg_data, reg_setting[conf_array.size - 1].delay);
				conf_sec_array.delay = reg_setting[conf_array.size - 1].delay;
			}
...
/* SHLOCAL_CAMERA_DRIVERS-> */
		...
		read_config.addr_type = read_config32.addr_type;
