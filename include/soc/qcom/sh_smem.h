...
typedef struct
{
    ...
    unsigned char       sh_camOtpData[28];        /* Camera Production adjustment Data */
    ...
    unsigned char       shpwr_traceability[22];      /* TODO:shpwr_traceability -> unused */
    ...
    unsigned char		shdiag_upperunit; 			 /* UpperUnit Status */
    unsigned char       sh_als_status;               /* Proximity, Ambient Light Sensor */
    unsigned short      sh_als_adj0[2];              /* Proximity, Ambient Light Sensor Alpha */
    unsigned short      sh_als_adj1[2];              /* Proximity, Ambient Light Sensor Beta  */
    unsigned short      sh_als_shift[2];             /* Proximity, Ambient Light Sensor Gamma */
    unsigned char       sh_als_clear_offset[2];      /* Proximity, Ambient Light Sensor */
    unsigned char       ir_offset[2];                /* Proximity, Ambient Light Sensor */
    unsigned char       sh_als_key_backlight;        /* Proximity, Ambient Light Sensor */
    unsigned int        sh_als_chksum;               /* Proximity, Ambient Light Sensor */
    unsigned char       fast_disp_info[3];           /* Boot Display Info */
    unsigned char       sh_memory_wpflg;             /* WP flag */
    signed short        sh_shub_offset_acc_xyz[3];   /* SensorHub Accelerometer Offset Data */
    unsigned char       sh_shub_offset_acc_flg;      /* SensorHub Accelerometer Offset Flag */
    signed short        sh_ssc_offset_acc_xyz[3];    /* SSC Accelerometer Offset Data */
    unsigned char       sh_ssc_offset_acc_flg;       /* SSC Accelerometer Offset Flag */
    unsigned char       shpwr_traceability_v2[30];   /* PWR:battery traceability v2 */
    int                 shsec_fuse_state[3];         /* shsec fuse state */
    unsigned char       sh_download_mode;            /* ramdump flag */
} sharp_smem_common_type;

#define SH_SMEM_COMMON_SIZE 30720

/*
 * AMSS specific SH_SMEM structure definition
 *
 * [The file path which must synchronize]
 *  adsp_proc/core/sharp/shsys/inc/sh_smem.h
 *  boot_images/core/sharp/shsys/inc/sh_smem.h
...
typedef struct
{
    unsigned char       sh_diagenc_gk[16];            /* Diag area encryption key */
    unsigned char       shdiag_KeyChashe[4][8];       /* MEID, MNF, BDAddr, WlanMacAddr Retention */
    unsigned char       sh_diagcal_data_buf[1*2048];  /* Diag area buffer */
    unsigned char       sh_rfcal_data_buf[81920];     /* RF adjustment value */
    unsigned char       sh_erase_efs_data_buf[128];   /* EFS corrupt detection */
} sharp_smem_specific_type;

#define SH_SMEM_SPECIFIC_SIZE 92160
...
FUNCTION sh_smem_get_specific_address

=============================================================================*/
sharp_smem_specific_type *sh_smem_get_specific_address( void );
